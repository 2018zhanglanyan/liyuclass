	...
	css:function(arg1,arg2){
		if (kQuery.isString(arg1)) {//是字符串的情况
			if (arguments.length == 1) {
				...
				if (this[0].currenStyle) {//兼容低级浏览器
					return this[0].currenStyle[arg1];
				}else{
...
kQuery.fn.extend({
	...
	off:function(eventName,fnName){
		...
		}else if (arguments.length == 1) {//传递参数名,解除所有事件名对应事件

		}else if (arguments.length == 2) {//传递参数名和函数名,解除所有事件名和和函数名对应事件
